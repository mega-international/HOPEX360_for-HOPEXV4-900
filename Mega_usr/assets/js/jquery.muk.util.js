...
function getD3ImpactData(sJson, bRootData) {
	...
	d3ImpactData.root = {
				...
				alwaysDisplay: (impactData.root.alwaysDisplay)?true:false,
...
function createForceGraph(sGraphID, sImpactDataLocation) {
  ...
  svg.append('defs').append('marker')
    ...
    contextControlContainer.append("<span style=\"margin-left:20px;\">Colourise by State:&nbsp;</span>");
    ...
    expandControl.click(function(){
		...
		function filter() {
			...
			store.nodes.forEach(function (n) {
				if (typeFilterList.includes(n.type_id) || (n.id === graph.root.id && graph.root.alwaysDisplay) || (n.fixed)) {
          if (contextFilterValue !== ""){
			...
			store.links.forEach(function (l) {
				...
				if ((typeFilterList.includes(l.source.type_id) && typeFilterList.includes(l.target.type_id)) ||
					((( (l.source.id === graph.root.id && graph.root.alwaysDisplay) || (l.source.fixed)) && (typeFilterList.includes(l.target.type_id) || (l.target.fixed))) ||
						((typeFilterList.includes(l.source.type_id) || (l.source.fixed)) && ( (l.target.id === graph.root.id && graph.root.alwaysDisplay) || (l.target.fixed))))) {
