...
function MgsDashboardDrillDown(dashboard, options) {
	...
	function init(dashboard, options) {
		...
		self.eOuterContainer = document.createElement("div");
		self.eOuterContainer.classList.add("col-md-12");
		self.eOuterContainer.classList.add("col-lg-12");
		self.eOuterContainer.classList.add("col-xl-12");
		self.eOuterContainer.style.marginBottom = dashboard.options.defaultBottomMargin;
		self.eParentContainer.appendChild(self.eOuterContainer);
...
var MgsDashboardFilterRenderers = {
	...
	"select": function (dashboardFilter) {
		...
		self.eSelect.onchange = function () {
			...
			if (self.currentValue === null) {
				...
				for (var i = 0; i < self.options.values.length; i++) {
					...
					if (self.options.values[i].id == self.currentValue && self.options.datastructureSelect) {
						// self.currentOption = self.options.values[i];
						console.log("self.currentOption = self.options.values[i] TRUE STRUCTURE " + JSON.stringify(self.currentOption));
						console.log("self.currentOption = self.options.values[i] TRUE STRUCTURE ");
						dataStructure = self.options.values[i]; 
						console.log("dataStructure TRUE STRUCTURE " + JSON.stringify(dataStructure));
						break;
					}
				}
			}
		...
		self.applyFilter = function () {
			...
			// console.log("filteredObjects count "+filteredObjects.length);
			self.dashboardFilter.dashboard.filteredData = filteredObjects;
		}

		self.updateData = function () {
			var data;
			// console.log("update Data and data "+JSON.stringify(data));
			// console.log("update Data and dataStructure "+JSON.stringify(dataStructure));
			// console.log("update Data and dataStructure "+dataStructure);

			// console.log("update Data and data "+data);
			// console.log("update Data and currentValue "+self.currentValue);
			// console.log("update Data and currentOption "+self.currentOption);
			// console.log("update Data and self "+self);
...
var MgsDashboardDrillDownRenderers = {
	"object-list": function (dashboardDrillDown) {
		...
		self.eParentContainer = dashboardDrillDown.eContainer;

		self.eHeaderContainer = document.createElement("div");
		self.eParentContainer.appendChild(self.eHeaderContainer);
		
		self.eHeaderText = document.createElement("h4");
		self.eHeaderText.style.display = "inline-block";
		self.eHeaderText.style.float = "left";
		self.eHeaderText.style.color = "#ffffff";
		self.eHeaderText.style.paddingLeft = ".5em";
		...
		self.eHeaderClose.style.display = "inline-block";
		//self.eHeaderClose.style.float = "right";
		self.eHeaderClose.style.cursor = "pointer";
		//self.eHeaderClose.style.margin = ".1rem";
		//self.eHeaderClose.style.textAlign = "center";
